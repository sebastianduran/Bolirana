...
for k in range (n_2+x, n):
    posx[k]= boxW+boxMarginW
    j=j+1
    
posy = list(range(n))
j=0
for i in range(0,n_2+x):
    posy[i]= (boxH*j)+boxMarginHMapa
    j=j+1
j=0
...
def h_mapa (x,y):
    ...
    gameDisplay.blit(pygame.image.load('rana.png'),(720+x,40+y)) 
    ...
    gameDisplay.blit(pygame.image.load('ranapeque.png'),(720+x,180+y))
...
def i_mapa():
    posXb = 2*boxW+500+boxMarginW
    monona = ["2 Botellas","1 Rana"]
    pygame.draw.rect(gameDisplay, white, [posXb,boxMarginHMapa,2*boxW,500],boxB)

    gameDisplay.blit(huecosFont.render("MONONA",1, white),(posXb+35,boxMarginHMapa+5))
    gameDisplay.blit(huecosFont.render(monona[0],1, white),(posXb+50-(2*len(str(monona[0]))),boxMarginHMapa+35))
    gameDisplay.blit(huecosFont.render(monona[1],1, white),(posXb+60-(2*len(str(monona[1]))),boxMarginHMapa+60))

    gameDisplay.blit(huecosFont.render("META",1, white),(posXb+50,boxMarginHMapa+250))
    gameDisplay.blit(metaFont.render(str(meta),1, white),(posXb+50,boxMarginHMapa+250+55))

    gameDisplay.blit(huecosFont.render("CASTIGO",1, white),(posXb+50,boxMarginHMapa+400+5))
    gameDisplay.blit(huecosFont.render("-"+str(castigo),1, white),(posXb+50,boxMarginHMapa+400+35))
